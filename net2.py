...
#       #pre1 and pre2 are MIB outputs, cur is current input
#         cur_size = cur.size()[2:]
#         # for modalties integrated
#         #CA after concatination over inputs from RGB_MIB and T_MIB
#         pre = self.ca1(torch.cat((pre1,pre2),1))
#         #modalties integrated upsampling
#         pre =self.conv1(F.interpolate(pre,cur_size,mode='bilinear',align_corners=True))
#         ##globalinfo upsampling and convoulution
#         global_info = self.conv_globalinfo(F.interpolate(global_info,cur_size,mode='bilinear',align_corners=True))
#         #current inputs from encoder
#         cur_feat =self.conv_curfeat(self.ca2(cur))
#         #sum of all three modalties integrated(pre)+current_input(cur_feat)+GI(global_info)
...
#MIB
    ...
    # def forward(self, pre1,pre2,cur1,cur2,global_info):
    def forward(self,cur1,cur2,global_info):
      #pre1 and pre2 are MIB outputs, cur is current input
        ...
        global_info = self.conv_globalinfo(F.interpolate(global_info,cur_size,mode='bilinear',align_corners=True))
        #current inputs from encoder
        cur_feat1 =self.conv_curfeat(self.ca2(cur1))
        cur_feat2 =self.conv_curfeat(self.ca2(cur2))
        #sum of all three modalties integrated(pre)+current_input(cur_feat)+GI(global_info)
        fus = cur_feat1 +cur_feat2 + global_info
        return self.conv_out(fus)
...
class FinalScore(nn.Module):
    def __init__(self):
        ...
        self.ca =CA(128)
        self.score = nn.Conv2d(128, 4, 1, 1, 0)
...
class Decoder(nn.Module):
    def __init__(self):
        ...
        # self.score_1=nn.Conv2d(128, 4, 1, 1, 0)
        # self.score_2 = nn.Conv2d(128, 4, 1, 1, 0)
    ...
    def forward(self,rgb,t):
        ...
        d1=self.gfb4_1(t[3],rgb[3],global_info)
        # d2=self.gfb4_2(global_info, rgb[4], t[3], global_info)
        #print(d1.shape,d2.shape)
        d3= self.gfb3_1(t[2],rgb[2],global_info)
        ...
        # score1=self.soft(score1)
        # score2 = self.score_2(F.interpolate(d5, xsize, mode='bilinear', align_corners=True))
        # score2=self.soft(score2)
        score =self.refine(d5,xsize)
#        score=1;,[   ]
        return F.softmax(score,1),score_global
...
class Mnet(nn.Module):
    ...
    def forward(self,rgb,t):
        ...
        t_f= self.t_net(t)
        score,score_g =self.decoder(rgb_f,t_f)
        return score,score_g

    def load_pretrained_modell(self):
